...
Replace runtime Bun shell template-tag usage in `packages/astrocoder/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/astrocoder/src` is removed except:
