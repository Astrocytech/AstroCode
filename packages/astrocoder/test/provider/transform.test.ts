...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const astrocoderModel = {
      ...openaiModel,
      providerID: "astrocoder",
      api: {
        id: "astrocoder-test",
        url: "https://api.astrocoder.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, astrocoderModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.astrocoder?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.astrocoder?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          astrocoder: { itemId: "msg_astrocoder" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              astrocoder: { itemId: "msg_astrocoder_part" },
    ...
    expect(result[0].providerOptions?.astrocoder?.itemId).toBe("msg_astrocoder")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.astrocoder?.itemId).toBe("msg_astrocoder_part")
