...
const AGENT_USERNAME = "astrocoder-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/astrocoder.yml"
...
jobs:
  astrocoder:
    if: |
      ...
      contains(github.event.comment.body, ' /astrocoder') ||
      startsWith(github.event.comment.body, '/astrocoder')
    ...
    steps:
      ...
      - name: Run astrocoder
        uses: anomalyco/astrocoder/github@latest${envStr}
        with:
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      try {
        ...
        console.log("astrocoder session", session.id)
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/astrocoder,/oc")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to astrocoder...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("astrocoder-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `astrocoder/${type}-${hex}-${timestamp}`
        }
        return `astrocoder/${type}${issueId}-${timestamp}`
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the astrocoder infrastructure after your response",
