...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for astrocoder.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal astrocoder sessions
...
astrocoder acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 astrocoder acp
```
...
- **Terminal Integration**: Full terminal support via astrocoder's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | astrocoder acp
```
...
### Mapping to astrocoder

ACP sessions map cleanly to astrocoder's internal session model:
