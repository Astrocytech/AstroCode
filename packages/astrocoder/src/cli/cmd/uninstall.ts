...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall astrocoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall AstroCoder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g astrocoder-ai",
      pnpm: "pnpm uninstall -g astrocoder-ai",
      bun: "bun remove -g astrocoder-ai",
      yarn: "yarn global remove astrocoder-ai",
      brew: "brew uninstall astrocoder",
      choco: "choco uninstall astrocoder",
      scoop: "scoop uninstall astrocoder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "astrocoder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using AstroCoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# astrocoder") || content.includes(".astrocoder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".astrocoder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".astrocoder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".astrocoder"))
    ) {
      continue
    }
