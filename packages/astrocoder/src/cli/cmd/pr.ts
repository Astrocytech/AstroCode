...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run astrocoder",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for astrocoder session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found astrocoder session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["astrocoder", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting astrocoder...")
        UI.println()

        // Launch astrocoder TUI with session ID if available
        const { spawn } = await import("child_process")
        const astrocoderArgs = sessionId ? ["-s", sessionId] : []
        const astrocoderProcess = spawn("astrocoder", astrocoderArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          astrocoderProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`astrocoder exited with code ${code}`))
          })
          astrocoderProcess.on("error", reject)
        })
      },
    })
  },
