...
test("discovers skills from .astrocoder/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".astrocoder", "skill", "test-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: test-skill
...
test("returns skill directories from Skill.dirs", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".astrocoder", "skill", "dir-skill")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `---
name: dir-skill
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const skillDir = path.join(tmp.path, ".astrocoder", "skill", "dir-skill")
...
test("discovers multiple skills from .astrocoder/skill/ directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir1 = path.join(dir, ".astrocoder", "skill", "skill-one")
      const skillDir2 = path.join(dir, ".astrocoder", "skill", "skill-two")
      await Bun.write(
...
test("skips skills with missing frontmatter", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".astrocoder", "skill", "no-frontmatter")
      await Bun.write(
        path.join(skillDir, "SKILL.md"),
        `# No Frontmatter
...
test("properly resolves directories that skills live in", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const astrocoderSkillDir = path.join(dir, ".astrocoder", "skill", "agent-skill")
      const astrocoderSkillsDir = path.join(dir, ".astrocoder", "skills", "agent-skill")
      const claudeDir = path.join(dir, ".claude", "skills", "claude-skill")
      const agentDir = path.join(dir, ".agents", "skills", "agent-skill")
      await Bun.write(
        path.join(claudeDir, "SKILL.md"),
...
`,
      ...
      await Bun.write(
        path.join(astrocoderSkillDir, "SKILL.md"),
        `---
name: astrocoder-skill
description: A skill in the .astrocoder/skill directory.
---
...
`,
      ...
      await Bun.write(
        path.join(astrocoderSkillsDir, "SKILL.md"),
        `---
name: astrocoder-skill
description: A skill in the .astrocoder/skills directory.
---
